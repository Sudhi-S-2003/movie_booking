...
import { Plus, MapPin, Search, Building2, Users, ShieldCheck, Map as MapIcon, ChevronRight, Layout } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useDocumentTitle } from "../../../hooks/useDocumentTitle.js";
import { DashboardPage } from "../../../components/dashboard/DashboardPage.js";

export const AdminTheatres = () => {
  useDocumentTitle("Theatres — OpsCenter");
  ...
  const fetchTheatres = async () => {
    try {
      setLoading(true);
      const res = await adminApi.listTheatres({ q: searchQuery });
  ...
  const handleAddTheatre = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Failed to register theatre infrastructure");
    }
  };
  ...
  return (
    <DashboardPage
      ...
      subtitle="Infrastructure & Partner Oversight"
      headerActions={
    ...
    >
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center min-h-[400px] gap-8">
            ...
            <p className="text-[10px] font-black uppercase tracking-[1em] text-gray-800">Establishing Data Link...</p>
        </div>
      ) : (
                ...
                <div key={theatre._id} className="group bg-white/5 border border-white/10 p-8 rounded-[40px] flex flex-col lg:flex-row items-stretch lg:items-center justify-between hover:bg-white/[0.08] transition-all gap-8 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-8 flex-1">
                        ...
                        <div className="space-y-1">
                            ...
                            <div className="flex flex-wrap items-center gap-4">
                                ...
                                <span className="flex items-center gap-2 text-gray-500 text-[10px] font-bold uppercase tracking-widest">
                                    <ShieldCheck size={12} className="text-green-500" /> Active Management
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-12 lg:px-12 border-l border-white/5 lg:border-white/5 border-t lg:border-t-0 pt-8 lg:pt-0">
                        <div className="flex flex-col items-start gap-1">
                            <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest">Operator Node</span>
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] font-black text-white">Verified Owner</span>
                            </div>
                        </div>
                        <div className="flex flex-col items-start gap-1">
                            <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest">Architecture</span>
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] font-black text-white">3 Available Screens</span>
                            </div>
                        </div>
                    </div>

                    <div className="flex items-center gap-4">
                        <button className="px-8 py-3 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 hover:text-white transition-all order-last lg:order-none">Manage Node</button>
        ...
        {showAddModal && (
          ...
          >
             ...
             >
                ...
                <h2 className="text-4xl font-black uppercase tracking-tight mb-4">Register Theatre Infrastructure</h2>
                <p className="text-gray-500 font-bold text-[10px] uppercase tracking-widest mb-12">Expanding the Global Cinematic Grid</p>

                <form onSubmit={handleAddTheatre} className="grid grid-cols-2 gap-12">
                   <div className="space-y-6">
                      <div className="space-y-2">
                         <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Network Name</label>
                         <input 
                            required
                            placeholder="IMAX Tech Node..." 
                      ...
                      <div className="space-y-2">
                         <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">City Hub</label>
                         <input 
                            required
                   ...
                   <div className="space-y-6">
                      ...
                      <div className="space-y-2">
                         <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Assign Owner (UUID/Email)</label>
                         <input 
                            required
                            placeholder="Search for owner node..." 
                   ...
                   <div className="col-span-2 mt-16 flex gap-6">
                      <button type="button" onClick={() => setShowAddModal(false)} className="flex-1 py-5 bg-white/5 border border-white/10 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all hover:bg-white/10 shadow-2xl">Abort Deployment</button>
                      <button type="submit" className="flex-[2] py-5 bg-accent-purple text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-2xl shadow-accent-purple/40 hover:scale-[1.02] active:scale-[0.98] transition-all">Authorize Infrastructure Integration</button>
                   </div>
                </form>
             </motion.div>
          </motion.div>
