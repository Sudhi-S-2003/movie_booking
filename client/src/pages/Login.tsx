...
export const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.requires2FA && res.tempToken) {
        // Persist token and destination safely in sessionStorage
        safeSession.setItem('temp_2fa_token', res.tempToken);
        safeSession.setItem('redirect_from', typeof from === 'string' ? from : '/');
        // Redirect cleanly to the dedicated 2FA verification route
  ...
  const handlePasskeyLogin = async () => {
    try {
      ...
      const emailQuery = loginId.trim() && loginId.includes('@') ? loginId.trim() : undefined;
      const res = await passkeyService.authenticate(emailQuery);

      // Authenticate successfully!
      setAuth(res.user, res.token, res.refreshToken || '');
      // Clear redirect path safely
      safeSession.removeItem('redirectPath');
      window.location.href = from;
    } catch (err: any) {
      setError(err.response?.data?.message || err.message || 'Passkey authentication cancelled or failed.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* PASSKEY BIOMETRIC SIGN IN TRIGGER */}
          <button
            type="button"
            onClick={handlePasskeyLogin}
            disabled={loading}
            className="w-full py-4 rounded-[20px] bg-[#141416]/50 border border-white/5 text-accent-blue hover:bg-accent-blue hover:text-white hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2"
          >
