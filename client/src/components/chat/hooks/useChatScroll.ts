...
/**
 * Manages scroll position, anchor restoration, and scroll-to-bottom FAB
 * for the chat message container — mirrors useChatScroll from support node.
 */
export function useChatScroll(conversationId: string | null): UseChatScrollReturn {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      frame = requestAnimationFrame(() => {
        ...
        setShowScrollBtn(distFromBottom > SHOW_FAB_PX);
      });
    };
  ...
  /** Save the current scrollHeight so we can restore after a prepend. */
  const saveAnchor = useCallback(() => {
    const el = containerRef.current;
    if (el) anchorRef.current = el.scrollHeight;
  }, []);

  /** After prepend: adjust scrollTop by the height delta to keep the view steady. */
  ...
  return useMemo(() => ({
    ...
    restoreAnchor,
    scrollToMessageEl,
  }), [showScrollBtn, highlightedId, scrollToBottom, saveAnchor, restoreAnchor, scrollToMessageEl]);
}
